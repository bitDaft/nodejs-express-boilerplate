...
export const standardErrorHandler = async (err, req, res, next) =>
  ...
  res.headersSent
    ...
    // ^ Please choose as necessary for your use case, by (un)commenting below 2 lines for generic response
    err.type === 'USER_INPUT'
...
export const normalErrorHandler = async (err, req, res, next) =>
  ...
  err instanceof Failure
    ...
    ? next(new Failure('Invalid data format provided', 'USER_INPUT'))
    : err.name === 'ValidationError'
    ? next(new Failure(err.message, 'VALIDATION_ERROR'))
    : err.name === 'UniqueViolationError'
    ? next(new Failure('Already exists', 409, 'UNIQUE_ERROR'))
    : err.name === 'TypeError'
    ? next(new Failure(err.message + '\n' + err.stack, 500, 'INTERNAL'))
...
export const standardSuccessHandler = async (req, res, next) =>
  ...
  res.headersSent
    ...
    ? // # This condition occurs when a success object type has been marked as internal
      ...
      next(
        new Failure(
          `Got 'INTERNAL' for route ${req.originalUrl}\n${JSON.stringify(req.x, null, 2)}`,
          500,
          'INTERNAL'
        )
      )
    : req.x
    ? // # This condition occures when an invalid value gets set as response and not a Response object
      ...
      next(
        new Failure(
          `Did not find properly formatted response data to send for route ${
            req.originalUrl
          }, instead got : ${JSON.stringify(req.x.null, 2)}`,
          500,
          'INTERNAL'
        )
      )
